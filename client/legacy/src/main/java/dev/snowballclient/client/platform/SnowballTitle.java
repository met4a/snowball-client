 ...
 * setScreen (see MinecraftClientMixin), so returning to the title from any other screen lands here too.
 */
public final class SnowballTitle {
	...
	public static Screen replace(Screen screen) {
		if (!(screen instanceof TitleScreen) || MinecraftClient.getInstance().isDemo()) return screen;
		...
		return new LegacyScreen(new TitleMenuView(client, new LegacyTitleActions()), null);
	}
}
